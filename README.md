...
`FRONTEND_URL` можно указывать списком через запятую, а завершающий `/` в значении backend теперь игнорирует при CORS-проверке.
...
Проверка backend в браузере:

- `GET /` возвращает JSON со статусом сервиса и доступными API-роутами
- `GET /health` возвращает простой healthcheck
- основные данные API находятся под `/api`, например `GET /api/tracks`
